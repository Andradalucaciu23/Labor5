...
public class EnrolledRepository{
    ...
    static final String QUERY_FIND = "SELECT * FROM ENROLLED WHERE courseID = ? AND studentID = ?";
    static final String QUERY_INSERT = "INSERT INTO ENROLLED VALUES(?, ?)";
    static final String QUERY_DELETE = "DELETE FROM ENROLLED WHERE courseID = ? AND studentID = ?";
    ...
    public HashMap<Long, Long> findAll() throws SQLException {
        ...
        while (resultSet.next()){
            map.put(resultSet.getLong("courseID"), resultSet.getLong("studentID"));
        }
